...
async def variable(var):  # sourcery no-metrics
    ...
    if exe == "get":
        ...
        if variable in heroku_var:
            await zed.edit("**⌔∮ تم تغيـر** `{}` **:**\n **- المتغير :** `{}` \n**- يتم الان اعـادة تشغيـل بـوت بد ثــون يستغـرق الامر 2-1 دقيقـه ▬▭ ...**".format(variable, value))
        else:
        ...
        await zed.edit(f"**⌔∮** `{variable}`  **تم حذفه بنجاح. \n**يتم الان اعـادة تشغيـل بـوت بد ثــون يستغـرق الامر 2-1 دقيقـه ▬▭ ...**")
...
async def dyno_usage(dyno):
    ...
    await asyncio.sleep(1.5)
    return await dyno.edit(
        "𓆩 𝗦َ𝙤𝙐!𝙧𝘾𝙚َ 𝘽ِ𝘿َ𝗧𝙃َ𝙤𝙉ِِ - 𝑫𝒀𝑵𝑶 𝑼𝑺𝑨𝑮𝑬 𓆪\n𓍹ⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧⵧ𓍻\n\n"
        f"**⌔∮ اسم التطبيق في هيروكو :**\n"
        f"**    - معرف اشتراكك ⪼ {Config.HEROKU_APP_NAME}**"
        f"\n\n"
        f" **⌔∮ مدة اسـتخدامك لبوت بد ثــون : **\n"
