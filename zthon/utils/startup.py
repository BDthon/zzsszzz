    ...
    try:
        if BOTLOG:
            Config.ZEDUBLOGO = await zedub.tgbot.send_file(
                BOTLOG_CHATID,
                "https://telegra.ph/file/e5900ea9548c628cae08b.mp4",
                caption="**•⎆┊تـم بـدء تشغـيل سـورس بـــدثــــون الخاص بك .. بنجاح 🧸♥️**",
                buttons=[(Button.url("𝗦َ𝙤𝙐!𝙧𝘾𝙚َ 𝘽ِ𝘿َ𝗧𝙃َ𝙤𝙉ِِ", "https://t.me/BDthon"),)],
            )
    except Exception as e:
        LOGS.error(e)
        return None
...
async def mybot():
    ...
    if bot_name.endswith("Assistant"):
        ...
        try:
            ...
            await bot.send_message("@BotFather", botname)
            await asyncio.sleep(1)
            await bot.send_message("@BotFather", f"- بـوت بـــدثـــــون المسـاعـد ♥️🦾 الخـاص بـ  {bot.me.first_name} ")
            await asyncio.sleep(3)
...
async def verifyLoggerGroup():
    ...
    if BOTLOG:
        ...
        _, groupid = await create_supergroup(
            "كـروب السجـل بدثــــون", zedub, Config.TG_BOT_USERNAME, descript, photozed
        )
    ...
    if PM_LOGGER_GROUP_ID != -100:
        ...
        _, groupid = await create_supergroup(
            "كـروب التخـزين بدثـــــون", zedub, Config.TG_BOT_USERNAME, descript, photozed
        )
